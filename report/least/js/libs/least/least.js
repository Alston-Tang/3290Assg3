		...
		return this.each(function() {
			...
			function intipreview(object, path, caption) {
				...
				close.on(
					'click',
					function() {
						settings.$container.find('.least-preview').slideToggle('slow');
			...
			$(this).find('li a').click(
				function(e) {
					...
					var $$ = $(this),
						path = $$.attr('href'),
						preview = settings.$container.find('.least-preview'),
					...
					if ( previewImg.length ) {
						...
						$$.addClass('active');
						settings.$container.find('.least-gallery li a.active').removeClass('active');
			...
			if(settings.random) {
				settings.$container.find('.least-gallery').each(function(){
					var ul = $(this),
						li = ul.children('li');
			...
			if(settings.scrollToGallery) {
				$(this).find('li a').click(
					function(e) {
						...
						$('html, body').animate(
							{ 
								scrollTop: settings.$container.offset().top
							}, 500 
						);
					}
				);
			...
			if(settings.HiDPI) {
				if(window.devicePixelRatio > 1) {
					/* var's */
					var image_thumb = settings.$container.find('img'),
						image_big = settings.$container.find('a');
